...
$totalDoctorPats   = max(array_column($topDoctors ?? [], 'nb_patients') ?: [0]) ?: 1;
$completionPct     = ($consultationCompletion['total'] ?? 0) > 0
    ? round((($consultationCompletion['completees'] ?? 0) / $consultationCompletion['total']) * 100)
    : 0;
...
// Helper to render initials safely if avatar.php is not loaded
function renderAvatar($id, $prenom, $nom) {
    $initials = strtoupper(substr($prenom, 0, 1) . substr($nom, 0, 1));
    return '<div class="w-9 h-9 flex-shrink-0 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 text-white flex items-center justify-center font-bold text-xs shadow-sm">' . htmlspecialchars($initials) . '</div>';
}
?>
...
<div class="space-y-6">
  ...
  <!-- Header -->
  <div class="mb-2">
    <h1 class="text-3xl font-extrabold text-blue-900 tracking-tight mb-1 font-headline">Tableau de Bord</h1>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
    ...
    <div class="kpi-card bg-white p-5 rounded-xl border-t-4 border-primary shadow-sm">
      <div class="flex items-center justify-between mb-3">
        ...
        <span class="text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full"><?php echo htmlspecialchars($doctorStats['growth'] ?? '+0%'); ?></span>
      </div>
      <p class="text-2xl font-extrabold text-on-surface"><?php echo number_format($doctorStats['total_doctors'] ?? 0); ?></p>
    ...
    <div class="kpi-card bg-white p-5 rounded-xl border-t-4 border-violet-500 shadow-sm">
      ...
      <p class="text-2xl font-extrabold text-on-surface"><?php echo number_format($totalPatients ?? 0); ?></p>
    ...
    <div class="kpi-card bg-white p-5 rounded-xl border-t-4 border-teal-500 shadow-sm">
      <div class="flex items-center justify-between mb-3">
        ...
        <span class="text-[11px] font-bold text-teal-600 bg-teal-50 px-2 py-0.5 rounded-full"><?php echo htmlspecialchars($consultationStats['status'] ?? 'Actif'); ?></span>
      </div>
      <p class="text-2xl font-extrabold text-on-surface"><?php echo number_format($consultationStats['active_count'] ?? 0); ?></p>
    ...
    <div class="kpi-card bg-white p-5 rounded-xl border-t-4 border-secondary shadow-sm">
      <div class="flex items-center justify-between mb-3">
        ...
        <span class="text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full"><?php echo htmlspecialchars($prescriptionStats['badge'] ?? 'Normal'); ?></span>
      </div>
      <p class="text-2xl font-extrabold text-on-surface"><?php echo number_format($prescriptionStats['total_prescriptions'] ?? 0); ?></p>
    ...
    <div class="kpi-card bg-primary p-5 rounded-xl shadow-lg text-white relative overflow-hidden">
      ...
      <?php $rev = ($prescriptionStats['total_prescriptions'] ?? 0) * 15 + ($consultationStats['active_count'] ?? 0) * 50; ?>
      <p class="text-2xl font-extrabold relative z-10"><?php echo number_format($rev, 0, ',', ' '); ?> DT</p>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
    ...
    <div class="lg:col-span-7 bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
      <div class="px-6 py-4 flex justify-between items-center border-b border-slate-100 bg-slate-50/60">
        ...
        <a href="/integration/dossier/admin/doctors" class="text-xs font-bold text-primary hover:underline">Voir tout →</a>
      </div>
          ...
          <tr class="hover:bg-blue-50/20 transition-colors">
            <td class="px-6 py-3.5">
              <div class="flex items-center gap-3">
                <?php echo renderAvatar($doctor['id_PK'], $doctor['prenom'], $doctor['nom']); ?>
                <div>
    ...
    <div class="lg:col-span-5 flex flex-col gap-5">
      <div class="bg-white rounded-xl shadow-sm border border-slate-100 border-l-4 border-l-tertiary overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 bg-slate-50/60 flex justify-between items-center">
          ...
          <a href="/integration/dossier/admin/consultations" class="text-[10px] uppercase font-bold text-tertiary">Historique</a>
        </div>
        <div class="p-4 space-y-2">
          <?php foreach ($recentConsultations ?? [] as $c): ?>
          <a href="/integration/dossier/admin/consultations/view?id=<?php echo $c['id_consultation']; ?>"
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-[11px] text-slate-500">Dr. <?php echo htmlspecialchars($c['medecin_prenom'] . ' ' . $c['medecin_nom']); ?> •
                <span class="<?php echo ($c['urgence'] ?? '') === 'Urgent' ? 'text-red-500' : 'text-tertiary'; ?> font-medium"><?php echo htmlspecialchars($c['urgence'] ?? 'Normal'); ?></span>
              </p>
            </div>
        ...
        <div class="relative z-10">
          ...
          <div class="space-y-2.5">
            <div class="flex justify-between items-center py-2 border-b border-white/10">
              ...
              <span class="text-sm font-bold"><?php echo (int)($prescriptionData['validated_today'] ?? 0); ?> ordonnances</span>
            </div>
            <div class="flex justify-between items-center py-2 border-b border-white/10">
              <span class="text-xs text-white/70">En attente signature</span>
              <span class="text-sm font-bold"><?php echo (int)($prescriptionData['pending_signature'] ?? 0); ?> dossiers</span>
            </div>
            <div class="flex justify-between items-center py-2">
              <span class="text-xs text-white/70">Actives</span>
              <span class="text-sm font-bold text-emerald-300"><?php echo (int)($prescriptionData['active_count'] ?? 0); ?></span>
            </div>
          </div>
          <a href="/integration/dossier/admin/ordonnances" class="mt-4 block w-full py-2 bg-white text-blue-900 rounded-lg text-xs font-bold hover:bg-blue-50 transition-colors text-center">Toutes les ordonnances →</a>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
      ...
      <div class="mb-6">
        <div class="flex items-end gap-3 mb-2">
          <span class="text-4xl font-extrabold text-on-surface"><?php echo $weeklyComparison['cette_semaine'] ?? 0; ?></span>
          <div class="mb-1">
            <?php
            $pct = $weeklyComparison['trend_pct'] ?? 0;
            $up  = $pct >= 0;
            ?>
            ...
            <p class="text-xs text-slate-400 mt-0.5">vs semaine passée (<?php echo $weeklyComparison['semaine_passee'] ?? 0; ?>)</p>
          </div>
        </div>
      ...
      <div>
        ...
        <div class="flex justify-between text-[11px] text-slate-400">
          <span><?php echo (int)($consultationCompletion['completees'] ?? 0); ?> complétées</span>
          <span><?php echo (int)($consultationCompletion['en_attente'] ?? 0); ?> en attente</span>
        </div>
      </div>
    ...
    <div class="flex flex-col gap-5">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-5 flex-1">
        ...
        <div class="space-y-3">
          <?php foreach (array_slice($topDoctors ?? [], 0, 5) as $i => $doc):
...
function viewDoctorPatients(doctorId, doctorName) {
  ...
  document.getElementById('doctorPatientsTitle').textContent = `Patients — ${doctorName}`;
