...
class AuthController
{
    ...
    public function forgotPassword(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($email)) {
                ...
                $result = $this->processPasswordReset($email);
                if ($result === false) {
     ...
     */
    private function processPasswordReset(string $email): bool|string
    {
        $db = $this->getDatabase();

        $stmt = $db->prepare(
            "SELECT id_PK, mail, prenom FROM utilisateurs WHERE LOWER(mail) = LOWER(:email) LIMIT 1"
        );
        $stmt->execute(['email' => $email]);
        $user = $stmt->fetch(\PDO::FETCH_ASSOC);
        ...
        try {
            ...
            error_log('Password reset token insert error: ' . $e->getMessage());
            return 'db_error';
        }
