...
$paginationBase = 'backOffice.php?controller=comment&action=view_post_comments'
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <?php foreach ($postComments as $comment):
            ...
            $deleteRedirect = 'backOffice.php?controller=comment&action=view_post_comments'
                . ($postId ? '&post_id=' . $postId : '')
                . '&page=' . $currentPage;
        ?>
        <div class="group bg-surface-container-lowest rounded-xl border-l-4 <?= $sm['color'] ?> <?= $sm['bg'] ?> p-5 shadow-sm hover:shadow-md transition-all duration-200">
            ...
            <div class="flex items-center justify-between mt-auto pt-3 border-t border-surface-container">
                ...
                <button onclick="showDeleteModal('backOffice.php?controller=comment&action=delete_comment&id=<?= $comment['id'] ?>&redirect=<?= urlencode($deleteRedirect) ?>', 'comment')"
